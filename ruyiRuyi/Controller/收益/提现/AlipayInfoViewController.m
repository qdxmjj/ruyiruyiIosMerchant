    ...
    [alert addAction:[UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        vtcodeView.block = ^(BOOL status) {
            ...
            if (status) {
                ...
                NSDictionary *params = @{@"storeId":[UserConfig storeID],@"realName":self.NameTextField.text,@"iDNumber":self.idNumberTextField.text,@"aliAccount":self.alipayAccountTextField.text};
                
                [JJRequest interchangeablePostRequest:@"/incomeInfo/bindingStoreAccountInfo" params:params success:^(id data) {
