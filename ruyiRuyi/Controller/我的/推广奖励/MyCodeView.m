...
- (void)layoutSubviews{
    ...
    self.nameLabel.frame = CGRectMake(90, 25, 100, 20);
    self.sexImageV.frame = CGRectMake(self.nameLabel.frame.size.width + self.nameLabel.frame.origin.x, 22, 25, 25);
    self.phoneLabel.frame = CGRectMake(self.nameLabel.frame.origin.x, 50, SCREEN_WIDTH - self.nameLabel.frame.origin.x - 10, 20);
...
- (void)setdatatoViews:(ExtensionInfo *)extensionInfo{
    ...
    [self.headImageV sd_setImageWithURL:[NSURL URLWithString:[UserConfig storeImgUrl]]];
    self.nameLabel.text = [UserConfig storeName];
    self.phoneLabel.text = [NSString stringWithFormat:@"tel:%@",[UserConfig phone]];
    self.codeImageV.image = [self getCode:extensionInfo.url];
    [self.midImageV sd_setImageWithURL:[NSURL URLWithString:[UserConfig storeImgUrl]]];
    
    if ([@(1) isEqualToNumber:[NSNumber numberWithInt:1]]) {
